...
def play_turn(user_message, history, state, last_question, finished):
    ...
    # Pure blocked world actions do not consume the question. Show the world notice
    # and keep the actual investigator question as the pending question.
    if state.question_count == before_count and state.last_world_notice:
        history.append({
            "role": "assistant",
            "content": f"[World] {state.last_world_notice}\n\n{last_question}",
        })
        return history, state, last_question, status_html(state), False
